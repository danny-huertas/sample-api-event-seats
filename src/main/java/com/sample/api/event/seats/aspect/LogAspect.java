package com.sample.api.event.seats.aspect;
...
public class LogAspect extends LogAspectBase {
    ...
    @Around("execution(* com.sample.api.event.seats.controllers.*.*(..))")
    ...
    @Around("execution(* com.sample.api.event.seats.service.*.*(..))")
    ...
    @Around("execution(* com.sample.api.event.seats.resource.dao.*.*(..))")
    public Object doAroundResourceHelper(ProceedingJoinPoint joinPoint) throws Throwable {
        return doAroundDebug(joinPoint, joinPoint.getArgs());
    }

    @Around("execution(* com.sample.api.event.seats.exception.*.*(..)) ")
